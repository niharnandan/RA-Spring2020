...
################################ Explanation ################################
# This code is very similar to 'MLE.py' but calculates 100 MLEs based on
# sample size 100 each and plots the histogram. It calculates MLEs based on
# three hypotheses:
...
# Note 1: We need 'simulation results 2.csv' to run the code, which is simulated by
#         'data simulation distribution.py'
# Note 2: Revise (model = 'none') to approprimate hypothesis chosen in the code 
#         (be consistent with simulations)
# Note 3: To plot histogram, change 'if False' to 'if True' in the code
# Minor Note: comment out 'count = 0', 'count += 1', 'print(count)', 'print(MLE_brute)'
#             if you don't want to print out the every step result of MLE
...
# open the simulation file, read it into a list of list, each element is a row of the file
with open('simulation results 2.csv','r') as f:
    file = [row for row in csv.reader(f.read().splitlines())]
file.remove(file[0]) # remove the first row, which is the row for names




################### calculate MLE for 100 simulations and plot histgram ###################
...
if model == 'alpha1_alpha2_lambda':
    MLE = [] # for carrying 100 MLEs 
    count = 0
    for i in range(100): # 100 simulation, each 100 trial
        start = i*100 # start row of ith simulation
        end = 100 + i*100 # end row of ith simulation
        simulation = file[start:end] # select the relevant row of ith simulation
        likelihood = lambda x: -log_likelihood(simulation,x[0],x[1],x[2],0) # objective function
        ...
        MLE.append(MLE_brute) # append the calculated MLE to the list
        count += 1
        print(count)
        print(MLE_brute)
        
    # write into csv file
    with open('MLE 3 parameters.csv','w') as csvfile:
        filewriter = csv.writer(csvfile, delimiter=',', quotechar='|',quoting=csv.QUOTE_MINIMAL)
        filewriter.writerow(['alpha1','alpha2','lambda'])
        for i in MLE:
            filewriter.writerow(i)
    csvfile.close()
    
    # plot histgram of each parameter to see how much they diverge from the true value
    alpha_1 = []
    alpha_2 = []
    k = []
    for i in MLE:
    ...
    # plot histgram of lambda
    if False:
        sns.set(color_codes=True)
        sns.distplot(k, kde=False, rug=True)
        plt.xlabel('lambda', size=16)

    # plot histgram of alpha 1
    if False:
        sns.set(color_codes=True)
        sns.distplot(alpha_1, kde=False, rug=True)
        plt.xlabel('alpha 1', size=16)

    # plot histgram of alpha 2
    if False:
        sns.set(color_codes=True)
        sns.distplot(alpha_2, kde=False, rug=True)
        plt.xlabel('alpha 2', size=16)
...
##### fit three parameters: alpha, lambda, stop_cost, with alpha1 = alpha2 #####   
    ...
    for i in range(100): # 100 simulation, each 100 trial
        ...
        simulation = file[start:end] # select the relevant row of ith simulation
        likelihood = lambda x: -log_likelihood(simulation,x[0],x[0],x[1],x[2]) # objective function
    ...
    # write into csv file
    with open('MLE equal alpha.csv','w') as csvfile:
        filewriter = csv.writer(csvfile, delimiter=',', quotechar='|',quoting=csv.QUOTE_MINIMAL)
        filewriter.writerow(['alpha','lambda','stop cost'])
        for i in MLE:
            filewriter.writerow(i)
    csvfile.close()
    ...
    for i in MLE:
        alpha.append(i[0])
        k.append(i[1])
        stop_cost.append(i[2])

    # plot the histgram of alpha
    if False:
        sns.set(color_codes=True)
        sns.distplot(alpha, kde=False, rug=True)
        plt.xlabel('alpha', size=16)

    # plot the histgram of lambda
    if False:
        sns.set(color_codes=True)
        sns.distplot(k, kde=False, rug=True)
        plt.xlabel('lambda', size=16)

    # plot the histgram of stop cost
    if False:
        sns.set(color_codes=True)
        sns.distplot(stop_cost, kde=False, rug=True)
        plt.xlabel('stop cost', size=16)
...
##### fit four parameters: alpha1, alpha2, lambda, stop_cost #####
    ...
    for i in range(100): # 100 simulation, each 100 trial
        ...
        simulation = file[start:end] # select the relevant row of ith simulation
        likelihood = lambda x: -log_likelihood(simulation,x[0],x[1],x[2],x[3]) # objective function
    ...
    # write into csv file
    with open('MLE 4 parameters.csv','w') as csvfile:
        filewriter = csv.writer(csvfile, delimiter=',', quotechar='|',quoting=csv.QUOTE_MINIMAL)
        filewriter.writerow(['alpha1','alpha2','lambda','stop cost'])
        for i in MLE:
            filewriter.writerow(i)
    csvfile.close()
    ...
    for i in MLE:
        ...
        alpha2.append(i[1])
        k.append(i[2])
        stop_cost.append(i[3])

    # plot the histgram of alpha 1
    if False:
        sns.set(color_codes=True)
        sns.distplot(alpha1, kde=False, rug=True)
        plt.xlabel('alpha 1', size=16)

    # plot the histgram of alpha 2
    if False:
        sns.set(color_codes=True)
        sns.distplot(alpha2, kde=False, rug=True)
        plt.xlabel('alpha 2', size=16)
