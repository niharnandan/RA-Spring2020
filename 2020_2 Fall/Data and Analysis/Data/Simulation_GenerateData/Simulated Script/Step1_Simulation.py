...
# Output:       Creates a cleaned dataset similar to Task2_Step2 from the human trials data
# Processing:   TBA
...
for name in range(65,65+no_files):
    ...
    # Create columns and initialize with 0
    ...
    for key,value in tqdm(step2_df.iterrows()):
        ...
        outcome = [x for x in temp if x != np.nan and (x == 5) or (x == 1) or (x == 0)] #Removing action apart from investigate
        add_evidence = [] if 5 in temp else [0] #Adding a dummy variable if accuse occurs in round 1
        outcome = [1 if i == 5 else 0 for i in outcome] + add_evidence
        for i in range(1,len(outcome)+1):
            step2_df['outcome_'+str(i)][key] = outcome[i-1] #Storing the evidence_found in the output column
